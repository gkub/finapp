...
def pull_database(database_path: Path | None = None) -> str | None:
    """Fetch remote DB before the app opens SQLite. Returns a warning, or None."""
    if not sync_enabled():
        return None
...
def push_database(database_path: Path | None = None) -> str | None:
    """Checkpoint SQLite and push after the GUI closes. Returns a warning, or None."""
    if not sync_enabled():
        return None
    path = (database_path or default_database_path()).expanduser()
    directory = path.parent
    ...
    staged = _git(["diff", "--cached", "--quiet"], directory)
    if staged.returncode == 0:
        return None
    stamp = datetime.now().astimezone().strftime("%Y-%m-%dT%H:%M:%S%z")
    committed = _git(["commit", "-m", f"snapshot {stamp}"], directory)
