...
def overdraft_headroom(account: Account) -> Decimal:
    ...
    if account.overdraft_limit is None:
...
def current_balance_sheet(session: Session, reporting_currency: str = "CAD", on_date: date | None = None) -> BalanceSheet:
    ...
    total_debt = sum((convert(item.current_balance, item.currency, reporting_currency, session, on_date) for item in debts), Decimal("0"))
    cash = operating_cash(session, reporting_currency, on_date)
    return BalanceSheet(cash, ordinary, investments, total_debt, ordinary + investments - total_debt)
